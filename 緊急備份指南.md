...
```json
{
  ...
  "message": "文件 leave_records.csv 不存在"
}
```
...
[BACKUP] 下載文件: leave_records.csv, 大小: 1024 bytes
[BACKUP] 文件下載完成: leave_records.csv
```
...
# 下載單一文件
.\emergency-backup.ps1 -ServerUrl "https://taixiang-server.onrender.com" -Action "download-leave"
.\emergency-backup.ps1 -ServerUrl "https://taixiang-server.onrender.com" -Action "download-personal"
```
...
# 下載單一文件
./emergency-backup.sh https://taixiang-server.onrender.com download-leave
./emergency-backup.sh https://taixiang-server.onrender.com download-personal
```

### 測試腳本 (Node.js)

**文件**: `test-backup-api.js`
...
# 測試 Render 部署
node test-backup-api.js https://taixiang-server.onrender.com
```
...
### 關鍵資訊
- **User-Agent**: `TaiXiang-Emergency-Backup-Tool`
- **狀態檢查端點**: `/api/backup/status`
- **下載端點**: `/api/backup/emergency-download?file={file-type}`
- **後端 URL**: `https://taixiang-server.onrender.com`

### 文件類型參數
- `leave-records` - 請假記錄 (leave_records.csv)
- `personal-data` - 個人資料 (personal_data.csv)
...
- ✅ 從 Persistent Disk (/mnt/data/) 讀取檔案

## 💡 使用提示

1. **檔案下載位置**: 
   - PowerShell: 下載到執行命令的當前目錄
   - 可以使用完整路徑指定下載位置，例如：`C:\backups\leave-records.csv`

2. **分兩行執行 PowerShell 命令**:
   ```powershell
   # 第一行：設定 headers
   $headers = @{ "User-Agent" = "TaiXiang-Emergency-Backup-Tool" }
...
3. **確認下載成功**:
   ```powershell
   # 查看檔案
   ls leave-records-backup.csv
   
   # 查看檔案內容（前 5 行）
   Get-Content leave-records-backup.csv -Head 5
   ```

4. **定期測試**: 建議每月測試一次備份功能，確保在緊急情況下可以正常使用
